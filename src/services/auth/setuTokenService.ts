...
class SetuTokenService {
  ...
  private async storeToken(token: SetuToken): Promise<void> {
    try {
      await secureStorage.setItem(TOKEN_STORAGE_KEY, JSON.stringify(token));
    } catch (error) {
  ...
  private async getStoredToken(): Promise<SetuToken | null> {
    try {
      const tokenData = await secureStorage.getItem(TOKEN_STORAGE_KEY);
...
export const setuTokenService = new SetuTokenService();
